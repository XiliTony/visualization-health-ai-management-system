...
public class RagConfig {
    ...
    /**
     * 创建并注册一个基于向量相似度的内容检索器 Bean。
     * 该检索器在运行时根据用户输入的问题生成嵌入向量，
     * 并从 Pinecone 向量数据库中检索最相关的若干文本片段（TextSegment），
     * 用于后续送入大语言模型生成答案。
     *
     * @return 配置好的 {@link ContentRetriever} 实例，支持最大 5 条结果、相似度阈值 0.75
     */
    @Bean
    public ContentRetriever contentRetriever() {

        // RAG，下面注释的内容已经创建了数据库就不用再运行了
        // 1. 加载文档
